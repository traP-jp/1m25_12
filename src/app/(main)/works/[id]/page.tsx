...
export default async function UserPage({ params }: { params: Promise<Params> }) {
 ...
 return (
  ...
  >
	...
	aria-label="Add tag" // スクリーンリーダー用の説明
	className="
	 mr-2 text-gray-900 dark:text-white  dark:hover:text-blue-300
  ...
  >
	<span className="i-material-symbols-download text-lg"></span>
		...
		<Link href={`https://q.trap.jp/channels/${path}`} size="sm" className="text-base">#{path}</Link>
		<p className="text-sm">{description ?? content}</p>
		<div className="flex flex-wrap items-center">
		
				<Link href="https://q.trap.jp/channels/team/graphics/progress" size="sm" className="!font-light !text-shadow-black !text-white bg-[#839404]/80 p-2 rounded-md ">#イラスト</Link>
	...
	aria-label="Add tag" 
	className="
	  text-gray-600 dark:text-blue-400 hover:text-orange-300 dark:hover:text-orange-300
  ...
  >
	...
	<Divider className="my-2" />
	  <div className="flex flex-col w-full">
		...
		<p className="text-xs text-gray-500 dark:text-gray-200 ml-auto">投稿日: 2025/5/2 22:55</p>
